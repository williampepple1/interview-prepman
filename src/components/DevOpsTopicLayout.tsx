...
const DevOpsTopicLayout: React.FC = () => {
  ...
  const topics = devOpsModule1Topics; // Only Module 1 is available for now
  ...
  const renderContent = () => {
    if (currentSubtopic) {
      return (
        <div>
          <div className="mb-6">
            <Link
              to={`/devops/module/${safeModuleId}/topic/${topicId}`}
    ...
    return (
      <div>
        ...
        <div className="prose prose-lg max-w-none dark:prose-invert prose-headings:text-gray-900 dark:prose-headings:text-white prose-p:text-gray-700 dark:prose-p:text-white prose-strong:text-gray-900 dark:prose-strong:text-white prose-code:text-gray-900 dark:prose-code:text-white prose-pre:bg-gray-100 dark:prose-pre:bg-gray-800 mb-8">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
            ...
            <div className="grid gap-4 md:grid-cols-2">
              {currentTopic.subtopics.map((subtopic) => (
                <Link
                  key={subtopic.id}
                  to={`/devops/module/${safeModuleId}/topic/${topicId}/${subtopic.id}`}
                    ...
                    {topicId === topic.id && topic.subtopics && (
                      <div className="ml-4 mt-2 space-y-1">
                        {topic.subtopics.map((subtopic) => (
                          <Link
                            key={subtopic.id}
                            to={`/devops/module/${safeModuleId}/topic/${topic.id}/${subtopic.id}`}
...
export default DevOpsTopicLayout; 